...
> 最后更新：2026-04-08（学科/星期偏好权重 50→200；班主任屏蔽学科加分；求解时限 300s→210s；Excel"全体班主任"合并格居中对齐）
...
1. **HC-1 — 覆盖**：每个活跃工作日的每个槽位必须恰好有规定人数。
   例外：强制日（`all_bz_required_days` 且 `mandatory_bz_count > 0`）跳过覆盖率约束——该日无求解器变量，由全体班主任集体覆盖。
...
- **SC-6** *（仅当 `mandatory_bz_count > 0` 时激活）* — 每个班主任变量三项叠加：
  - `penalty_bz_second_duty`（−30）：任意天微弱处罚（强制日已算第一次，求解器分配的任何值班均为第二次）。
  - `penalty_bz_second_duty_weekend`（−100）：周五/周日的额外较大处罚。
  - `pref_bz_mon_thu_after_mandatory`（+310）：周一至周四奖励。
  - 净效果：周一至周四 = SC-4(−200)+SC-6(+310)+二次(−30) = **+80**；周五/周日 = 周末(+100)+二次(−30)+周末处罚(−100) = **−30**。
  - `mandatory_bz_count == 0` 时整块 SC-6 跳过，原系统零改动。
